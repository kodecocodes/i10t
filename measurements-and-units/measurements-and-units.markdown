...
When dealing with real-world units in code it's easy just to use one of the built in numeric types and just try and remember everywhere which units you're supposed to be using. When it comes to displaying things to the user, you then do something like this:

```swift
distanceLabel.text = "\(distance) miles"
```

But that looks wrong if there is only one mile, or the mile has a lot of decimal places, so you add some more code. Then it doesn't localize properly, so you add some _more_ code. Then people want to see their distances in kilometers, so you add a preference and some conversion logic. 

If this is a familiar story, you're in luck. The Foundation framework has some exciting new additions this year for solving exactly these problems. You're going to learn about `Measurement` and `Unit`, and how they allow you to:

- Get rid of fiddly conversions
- Use strongly typed values that prevent you from using yards when you meant Kelvin
- Show values to the user in terms they understand
- Get all this power for your _own_ measurements and units. 

Another exciting addition to Foundation this year is the date interval - a period from one `Date` to another. Again, this represents a problem that is commonly solved by having to write a lot of code that you probably don't test, and almost certainly don't test for international users. Date intervals and the associated date interval formatter will make your life easier when you need to work with dates - and let's face it, dates are hard, so all help is welcome! 
...
The `Measurement` struct doesn't sound like much - it has two properties, a `value`, which is a `Double`, and a `unit`, which is a `Unit`. `Unit` is even more minimal - all that has is a `symbol`, which is just a `String`. 

This minimal implementation gives you some benefits - given a `Measurement`, you'd know right away what units it was in, and be able to format it for display using the symbol that comes with the unit. But the real power of the system comes from two things: 
...
First, the `Unit` subclasses. Foundation gives you the `Dimension` subclass, which is for representing units that have a dimension, like length or temperature. Dimensions have a base unit and a converter that can interpret the value to and from the base unit. It's an abstract class, which means you're expected to subclass it to make your own dimensions. A wide variety are already included in Foundation. 

As an example, there is a `UnitLength` class. This is a subclass of `Dimension` used to represent lengths or distances. The base unit is the meter, which is the SI unit for length. SI stands for Système International, the internationally agreed system of measurements. Readers from the US, brace yourselves for learning that you are measuring almost everything incorrectly. 
...
A marathon is 26 miles 385 yards. This awkward number comes from the 1908 London Olympics, where the organisers planned a course of 26 miles, then had to add an extra bit so that the race would finish neatly in front of the Royal Box. Do you know what 26 miles 385 yards is in "Decimal miles"? I don't, and now, I don't have to. 

To create a measurement of a marathon, add this to the playground:
...
As you might expect, `triathlon` shows up in the results sidebar in meters, which isn't particularly useful. Generics can help you here. `Measurement` instances that have a `Unit` that is a subclass of `Dimension` have a useful extra feature: they can convert to other units, or be converted to other units. To see the triathlon total in miles, add this line:
...
> **Note:** If you _have_ got a nuclear physics degree, congratulations on your change of career, but please don't get upset about errors or simplifications in this chapter. 
...
That gives you a very small number of joules. You don't run a nuclear reactor with a single atom of uranium, though, you use a rod or pellets of the stuff. So how much energy is contained in a given weight of uranium? 
...
This formula uses a special number called Avogadro's number which defines the number of atoms in a _mole_, which is in turn approximately the number of amus that weigh one gram. Don't worry too much about understanding the formula - but note that we can pass in any `UnitMass` we like and get a value out of the other end. 

Use this function to get the number of atoms in 1lb of uranium, then multiply that by the value you obtained earlier for the energy released by a single atom:
...
In the results sidebar in the playground, often the numbers are displayed with lots of decimal places or exponentials. In the next section you're going to take control of presenting your measurements with `MeasurementFormatter`.
...
> **Note:** Unlike date and number formatters, measurement formatters are one-way. You can't use them to take a string like "1kWh" and turn it into a `Measurement`. 

You have a lot of control over the presentation of your measurements - you can specify a locale, the style of TODO BLAH BLAH
...
Measurement formatter has a property, `UnitOptions` which acts as a sort of grab bag of random options which didn't really fit elsewhere. Currently there are only three options in there, one of which specifically relates to temperature. 
...
Remember earlier, when you added miles to yards to nautical miles to kilometers, and the answer was given in meters? The number of meters was quite high, and to present a meaningful value to the user you would have to have a conversion step to a more sensible unit, and you may need to write code to determine what that more sensible unit should be. Measurement formatters can do this for you, for some kinds of unit. 
...
formatter.string(from: run)
```
The default value is `.medium`, which prints the symbol of the unit in use. There is no public API to provide extended or shorter names or symbols for your own units. 
...
TODO: One of you lot might want to add a yo dawg meme here about formatting while you format, but I'm not doing it
...
This is a little complicated to understand. Here's a diagram:
...
It seems like in Olde England people would have a thing to be measured, look around them, pick the first thing they saw and use that as a unit. Hence we have poppyseed, finger, hand, foot, rod, chain, and many more. 
...
2. These are the two properties needed to perform logarithmic conversions
...
There are two methods you must override to make a working converter - to convert to and from the base unit. These calculations are the standard methods for dealing with logarithms - it isn't important for the tutorial that you follow the math. Add the following methods to your converter class:
...
This has created a new `Dimension` subclass. There's a single instance, decibels. This is set up just like the `chains` that you added to `UnitLength`. Decibels is going to be the base unit, so to "convert" it doesn't need to do any work - the linear converter with a coefficient of 1 will do the job.
...
let next24Hours = DateInterval(start: today, 
...
If two date intervals start at different times, the one with the latest start date is larger - the lengths of the intervals are not compared.
...
It turns out you've won a surprise holiday! It's two weeks long, and it starts right now! (Now being 1pm on Friday. You have to allow a little poetic license here because you could be following this tutorial at any point in the week, but the code has to work the same way!). Add this code to represent your holiday:
...
Date interval formatter isn't too exciting - you configure it just like a `DateFormatter`, and it then applies the format to the start and end dates of the interval and puts a hyphen in between them. But it does save you having to do that step yourself. 
