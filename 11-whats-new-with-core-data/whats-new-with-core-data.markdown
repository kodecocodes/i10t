...
The app is TaterRater, which is incredibly handy for creating ratings notes about your favorite varieties of potato. Later, you’ll make a greater TaterRater using Core Dater — er, _Data_.
...
You only had one context that would talk to the persistent store coordinator. Typically that was a background queue context whose only job was to perform saves.

Under that was the main thread context, which represented the “truth” of your app. Subsequent background operations or foreground editing contexts would be children of that main thread context. 

This was necessary because the persistent store coordinator and SQL store could not handle multiple readers or writers without having to use locks.

In iOS 10, the SQL store has been improved to allow multiple readers and a single writer, and the persistent store coordinator no longer uses locks.

This means that a context hierarchy now looks more like this: 
