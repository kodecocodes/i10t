...
This is required for when you request access to the camera, microphone and photo library later on. 

`AVFoundation` contains a specialist `CALayer` subclass, `AVCaptureVideoPreviewLayer`, which can be used to show the user what the camera is currently seeing. There's no support for this in interface builder, so you need to make a `UIView` subclass to deal with it. 

Create a new file, using the **Cocoa Touch Class** template. Call the new class **CameraPreviewView**, and make it a subclass of **UIView**. Replace the contents of the file with the following: 
...
Open the assistant editor and control-drag into **ViewController.swift** to make a new outlet from the camera preview view. 
...
An `AVCaptureSession` is the object that deals with handling the input from the cameras and microphones. Most of the methods relating to capture and processing are asynchronous and can be handled on background queues, so you create a new queue to deal with everything session-related. The capture device input represents the actual camera / microphone that is being used to capture the audio and video. Now add the following code to `viewDidLoad()`: 
...
1. `beginConfiguration()` tells the session that you’re about to add a series of configuration changes. They get committed at the end. There are a series of presets that can be applied
2. Create a device representing the front facing camera.
3. Create a device input representing the data that the device can capture
...
2. Create an `AVCapturePhotoSettings` object which contains the information about how the photo should be taken (flash mode, etc.)
3. Tell the output object to capture a photo, passing in the settings and a delegate object. 

First, the UI. Open **Main.storyboard** and make things look a little more camera-like by setting the main view’s background color to black, and the **Global Tint** color (in the File Inspector) to a tasteful orange. Set the camera preview view's background color to black as well.
...
Add constraints to center the stack view horizontally in the superview, then pin it to the **top** of the superview with a spacing of **5**, and the **bottom** with a spacing of **20**. Update any frames that Xcode is complaining about, and you should end up with something like this:
...
3. The photo settings is created and configured. For basic JPEG capture, there aren’t many things to configure, as you can see.  
...
1. You’ll supply closures to be executed at key points in the photo capture process. These are when the capture begins and ends, then when the captured image has been processed and everything is complete. 
2. A property to hold the data captured from the output
3. The init method ensures that a completion closure is passed in - the other event closures are optional.
...
Some more very wordy methods, that just execute the closures that have been passed in for these event points. Build and run and take some more photos, and you’ll see a camera-like animation each time you hit the button. 

The built-in camera app has a nice feature where it shows a thumbnail of the last photo you took in the corner. You’re going to add that to PhotoMe. You may have noticed the `previewPhotoSampleBuffer` parameter in the extraordinarily long delegate method that is called when the photo sample buffer is processed. This gets used to make an embedded preview in the JPEG file that is created, but you can also use it to make in-app thumbnails. 
...
Then add this code to the end of the `...didFinishProcessingSampleBuffer...` method:
...
This tells the photo settings that you want a 160x160 preview image to be created, in the same format as the main photo. Still in `capturePhoto()`, add the following code after you’ve created the delegate object:
...
What you’ve done so far is mostly a slightly nicer version of what you could already do with `AVCaptureStillImageOutput`. In the next two sections you’ll get on to totally new stuff: taking live photos, and editing them! 

Open **Main.storyboard** and drag a new **Horizontal Stack View** into the **Option Stack**, above the **Thumbnail Stack**. Call the new stack view **Live Photo Stack** and set the **Spacing** to **5**. Drag in a switch and a label, set the label text to **Live Photo Mode** and the text color to white, and set the switch to **Off**. This will control your capture of live photos.
...
Open **ViewController.swift** and add the following code to `prepareCaptureSession()`, just after the video input device is created:
...
In `capture(_: didFinishCaptureForResolvedSettings:error:)`, just after you call `addResource` on the creation request, add this code:
...
Then in `capturePhoto()`, where you assign all the other closures, add this new one:
...
Previously, when you edited Live Photos they lost the accompanying video and became, well, Dead Photos. In iOS 10 you can apply the same range of edits to a Live Photo as you can to any photo, and those edits are applied to every frame of the video as well. What’s even better is that you can do this in your own apps, too. You’re going to apply a cool core image filter to live photos taken in PhotoMe. 
...
Set the main view's background color to black. Drag in a vertical stack view and drag the left, top and bottom edges out to reach the edges of the screen. Add constraints to pin those edges. Set the spacing to **20**. 
...
4. In your case you are applying the same `CIFilter` to each frame, which is the Comic Effect filter. You could put any combination of core image filters in here, or perform any other manipulations you can think of.
